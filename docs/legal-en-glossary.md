...
| 사업자 정보 칸 폭 | 딱지 160px 고정 · 두 벌 사이 56px | **글자 길이에 맞춘다** — 딱지 단은 단마다 제 가장 긴 딱지(max-content), 왼쪽 값 단은 주소만큼 + 남는 폭 | 208px 고정이면 주소가 세 줄로 접혀 행 간격이 들쭉날쭉했다(사용자 지적). 1440px 실측 177 · 327 · 207 · 120px, 값 모두 한 줄 · 행 간격 49px로 한국어판과 같다 |
