...
| `company.address` | 대전광역시 유성구 테크노3로 24(관평동) | 24, Techno 3-ro, Yuseong-gu, Daejeon, Korea | ✅ 2026-09-11 확인 — 아래 주의 |
...
> **주소** — 국문 · 영문 모두 도로명이다. 국문은 처음에 지번(`관평동 1311`)이었다가 영어판 작업을 마친 뒤
> 도로명 `대전광역시 유성구 테크노3로 24(관평동)`으로 바꿨다(2026-09-11, 사용자가 준 표기 그대로).
...
| 사업자 정보 칸 폭 | 딱지 **130px** · 두 벌 사이 **28px** · 상자 높이 34px (두 벌 같은 비율, 주소 한 줄 — 2026-09-11 사용자 결정 A안) | 딱지 **160px** · 두 벌 사이 56px, 상자 높이는 모두 두 줄 높이 52px. 두 벌 같은 비율은 국문과 같고 주소는 두 줄 | 영어 딱지 'Business registration number'는 130px에서 세 줄로 접혀 상자 밖으로 넘친다 — 영어만 160px. 딱지를 값 위로 쌓는 경계는 두 판 모두 **640px**(처음 520px). 전화 딱지 영문은 `Tel`(사용자 지정), 주소 딱지는 `Address` 유지 |
...
| 6 | **블라인드 역번역** | 원문을 보지 못한 번역자가 영어판만 보고 한국어로 되돌리고, 다른 검토자가 원문과 맞댄다. 고친 문장은 다시 블라인드로 | 넓어짐 · 좁아짐 · 세기 0, 두 가지로 읽힘은 원문에도 있는 것만. 기록 `docs/legal-en-blind-review.md` (2026-09-15 네 문서 692단위 — 16곳 고침) |
