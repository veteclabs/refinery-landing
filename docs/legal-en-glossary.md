...
| 표 칸 폭 | 자동 배분 | **한국어판 1440px 칸 비율을 그대로 th width로 고정** (2026-09-11 사용자 결정) | 두 판이 같은 표로 읽히게. 영어 머리글은 두 줄 허용(`:lang(en) .legal-table th`) — 한 줄 고정이면 긴 머리글이 칸을 밀어 비율이 어긋난다. 한국어 표를 고치면 영어 비율도 다시 잰다 |
| 사업자 정보 딱지 단 | 160px | 208px (가장 긴 'Business registration number' 182 + 24) | 520px 이하는 딱지를 쌓아 해당 없음. 주소가 세 줄로 접히는 것은 두 판의 카드 구조를 같게 두는 쪽을 택했다 |
