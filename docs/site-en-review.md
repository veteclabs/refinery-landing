...
### 6.25 블로그 · 「예지보전은 무엇을 예측하는가」 — 20단위

문장 누락도 뜻이 바뀐 곳도 없었다. **한 곳만** 고쳤다.

| 단위 | 한국어 | 바꾸기 전 | 바꾼 뒤 |
|---|---|---|---|
| [16] | **현장이** 움직이지 않습니다 | does not move **anyone** | does not move **the shop floor** |

23번과 같은 유형이다 — 원문 「현장」이 `anyone`으로 넓어졌다.
`does not move anyone`도 영어로는 자연스럽지만, 같은 글 묶음에서 두 번 나온 유형이라 맞췄다.

그대로 둔 것: [3] `plants`(원문 「**많은** 현장」), [9] `quality`→'정확도',
[1] 원문이 의문형(「무엇을 예측**하는가**」)인데 영어가 명사절 — 제목으로는 통한다.

### 6.26 블로그 · 「산업 데이터를 자산으로 만드는 5가지 원칙」 — 30단위

가장 긴 글인데 **30단위 중 28곳이 깨끗했다.** 두 곳만 고쳤다.

| 단위 | 한국어 | 바꾸기 전 | 바꾼 뒤 |
|---|---|---|---|
| [2] 요약문 | **신뢰할 수 있는** 의사결정 근거로 | something you can **actually decide on** | something you can **trust and decide on** |
| [30] | 우리 **현장**에 어떻게 적용할지 | your own **site** | your own **operation** |

[2]는 글 전체가 신뢰를 다루는데(첫머리 「connection and **trust**」, 원칙 1 「the minimum condition for **trust**」)
요약문에만 그 말이 없었다. **블로그 목록 카드에도 함께 실린다.**

[30]의 `site`는 **회사소개 [20]에서 같은 이유로 고친 것**과 같다(번역기가 '사이트'로 음차).
23번은 `your own process`라 문제가 없었다.

그대로 둔 것: [17] `the same period last season` → '지난 시즌' — 원문 「지난 계절」이고
블라인드에서도 모호하다고 표시했으나, 제조에서 계절성 패턴을 가리키는 표준어다.

### 6.27 블로그 · 「Refinery 블로그를 시작합니다」 — 16단위

| 단위 | 한국어 | 바꾸기 전 | 바꾼 뒤 |
|---|---|---|---|
| [3] | (한 문장) | **We are starting the Refinery blog.** This is where… | This is where… |
| [6] | 현장 데이터 **모범사례** | Field data **practices** | Field data **best practices** |

[3]은 원문에 없는 문장이 앞에 붙어 있었고, **제목(`The Refinery blog starts here`)과 같은 말을 두 번** 했다.
[6]은 이 글의 태그가 `#best practices`(#모범사례)이고 5가지 원칙 글로 이어진다.

그대로 둔 것: [4] `We are a team`(원문 「Refinery팀은」), [12] `anyone`(원문 「분들께」).

---

## 7. 번역기 역번역(B안) 마무리

**26페이지 · 1,119단위를 모두 대조했다.**

| 묶음 | 페이지 | 고친 곳 |
|---|---|---|
| 1/4 랜딩 · 회사소개 · 개념 · 허브 | 7 | 28 |
| 2/4 산업 | 7 | 40 + 공통 19 |
| 3/4 유즈케이스 | 7 | 17 + 카드 제목 25 |
| 4/4 블로그 | 5 | 9 |
...
| 유형 | 사례 |
|---|---|
| **제품명이 일반명사로** | `Refinery Industrial`→정유 산업 · `Refinery … plant site`→정유 공장 · `Starting the Refinery`→블로그 이름 |
| **원문에 없는 단정** | `never` · `any` · `every` (랜딩 → 산업 → 블로그 순으로 세 차례 걸러냈다) |
| **원문 「이상」이 `faults`로** | 8개 페이지 12곳 |
| **원문 「냉열」에서 열이 빠짐** | 식품 · 기계 5곳 |
| **한 낱말이 다른 뜻으로** | `draw`→성형 · `heating`→난방 · `microclimate`→미기후 · `the floor`→의회 · `case`→사례 · `site`→웹사이트 |
| **세 곳을 함께 안 봄** | `tagline` · `heroTitle` · `cardDesc` (예지보전 · 전력관리) |

### 배운 것

1. **한 유형을 고칠 때 전수로 훑는다** — `faults` · `site data` · `every`가 모두 한 곳씩 고친 뒤 다른 페이지에 남아 있었다
2. **일괄 치환 뒤 대소문자를 확인한다** — 문장 첫머리 `Site data` 한 곳이 남았다
3. **고칠 때 넣은 말도 다시 검증한다** — 누락을 메우며 넣은 `heating equipment`가 '난방 장비'로 읽혔다
4. **문법 자리가 위험도를 가른다** — `the floor`는 주어 자리에서만 '의회'가 됐다
5. **`before`절은 어순을 뒤집는다** — 낱말로 못 막고 `when`으로 바꿔야 한다
6. **역번역이 길어지면 영어에 원문에 없는 요소가 있는지 본다** — `each` 하나가 역번역을 두 배로 불렸다

## 8. 최종 종합 검증 — 원어민 관점

§6까지는 번역기 역번역으로 **원문과 어긋나는 곳**을 찾았다. 마지막으로 남은 물음은
"원어민 고객이 읽을 때 문법·용어·표현이 성립하는가"였다.

### 8.1 방법

1. **기계 검사** — 최신 빌드로 ko/en 1,124단위를 다시 짝지었다.
   단위 수 불일치 0, 같은 한국어 → 다른 영어 1건(`AI 에이전트`의 단·복수, 정상), 고유명사 41종 일관.
   → **문단·문장 단위의 누락은 없다.** 남은 문제는 문장 안쪽이었다.
2. **원어민 검토 4묶음** — 한국어 원문 접근을 차단한 검토자에게 영어만 주고
   문법·가독성·전문 용어·부자연스러운 표현만 보게 했다("억지로 찾지 말라"를 명시).

   | 묶음 | 단위 | 글자 |
   |---|---|---|
   | A 랜딩군 | 173 | 14,057 |
   | B 산업 7페이지 | 632 | 30,855 |
   | C 유즈케이스 7페이지 | 223 | 17,380 |
   | D 블로그 5편 | 96 | 17,476 |

3. **원문 대조로 걸러내기** — 143건 중 **원문을 바꾸게 되는 제안은 기각**했다.
   원문 접근을 막았기 때문에 "영어로는 더 자연스럽지만 원문에 없는 말"이 섞여 나온다.
   기각 예: `catches defects before they happen`(영어에서도 흔한 표현),
   `We plan to expand English support`(원문이 '확대'), 데모 영상 문구(손대지 않기로 한 것),
   전자 페이지 UPS 중복(국문도 동일).

### 8.2 A군 — 원문과 내용이 어긋난 11건

역번역으로는 안 잡혔다. 역번역해도 그럴듯한 한국어가 나오기 때문이다.

| 곳 | 원문 | 고치기 전 | 문제 |
|---|---|---|---|
| 랜딩 AI 답변 | 대화형으로 답변합니다 | `…infer causes and answers` | '대화형으로' 누락 + `causes and answers`가 명사 둘로 읽힘 |
| 랜딩 자동화 카드 | 사람 없이도 돌아가는 **현장** | `Automation with no hands` | '현장' 통째 누락 |
| 랜딩 절차 카드 | **운영** 절차 | `Procedures set by dialogue` | '운영' 누락 |
| 랜딩 통합 제목 | **끊김 없이** | `without a break` | 오역 — 영어로는 '쉬지 않고' |
| 회사소개 서명 | …만든 제품 | `… in the field — Refinery` | 원문에 없는 'Refinery' 추가 |
| 산업 CTA ×7 | **Refinery 팀이** 함께 | `…, together.` | 'Refinery 팀' 누락 |
| 유즈 예지보전 | 배터리로 **오래** 돌고 | `the battery lasts` | '오래' 누락 |
| 유즈 전력품질 | 언제 **어느 설비 근처에서** | `when and where` | '설비' 누락 |
| 자동차·식품 원단위 | 에너지 **원단위** | `derive intensity` | `energy` 빠져 뜻이 안 열림 |
| 블로그 데이터원칙 | **결국** 아무도 보지 않게 | `— until nobody looks` | 인과를 시간으로 뒤집음 |
| 위 + 유즈 예지보전 | **알람** 피로 | `alert fatigue` | 공장 표준어는 `alarm fatigue`(ISA-18.2) |

### 8.3 판단이 갈린 곳

- **`현장`을 무엇으로 옮기는가** — 세어 보니 `site` 48곳 · `field` 31곳 · `floor` 6곳으로
  **`site`가 이미 최다 용례**였다. 카드 제목처럼 **맨명사 자리**에 선 `a field`는
  역번역에서 '분야'로 돌아온다(수식어 자리의 `field data`는 안전하다).
  `site`는 역번역에서 '사이트'가 되지만 이는 한국어 외래어 사정이고 영어에는 중의성이 없다.
  → 결국 구두점 없이 `Automation that runs the site without people`로 갔다.
- **em dash(—)** — 영어 랜딩 카피에서 흔한 장치이고(가운뎃점 `·`과 다르다) 우리 비중은
  1,124단위 중 33곳(2.9%)으로 절제된 수준이다. 다만 **같은 줄의 형제 카드 5개가
  구두점이 하나도 없어서** 자동화 카드만 대시를 넣으면 눈에 걸린다 → 구두점 없는 쪽을 골랐다.
  본문 삽입구(전력품질 카드)도 대시 없는 문장으로 바꿨다.
- **짧은 하이픈(-)으로 대체 불가** — 영어에서 `-`는 낱말을 붙이는 기호다(`3-axis`).
  낱말 사이에 공백을 두고 단독으로 쓰면 오타로 보인다. 용어집 §0의 `—`(양옆 공백)를 유지한다.

### 8.4 남은 묶음 (미반영)

- ~~**B 문법**~~ — 24건 반영 완료(8.6 참고)
- ~~**C 전문 용어**~~ — 12건 반영, 3건 보류(8.9 참고)
- ~~**D 직역투**~~ — 81곳 반영, 재점검에서 1곳 되돌림(8.11~8.13 참고)
- ~~**E 표기 통일**~~ — 반영 완료(8.10 참고)
- ~~**F 사용자 판단 7건**~~ — 3건 반영, 4건 유지(8.15 참고)
- ~~**말투**~~ — 요청 4항을 빠뜨렸다가 뒤늦게 처리(8.14 참고)

### 8.5 배운 것

7. **역번역은 누락을 못 잡는다** — 영어에서 빠진 말은 역번역에도 없으니 한국어가 자연스럽게 읽힌다.
   A군 11건은 원문을 모르는 원어민이 "문장이 안 읽힌다"고 짚어야 드러났다.
8. **원문을 모르는 검토자는 원문을 바꾸려 한다** — 143건 중 실제로 쓸 수 있는 건 절반 남짓이었다.
   두 검증(원문 있는 쪽·없는 쪽)을 겹쳐야 누락과 비문이 동시에 잡힌다.
9. **역번역이 어색해도 영어가 맞을 수 있다** — `site`→'사이트', `energy intensity`→'에너지 강도'는
   한국어 외래어·직역 사정이지 영어 문제가 아니다. 역번역만 보고 되돌리면 영어를 망친다.
10. **낱말의 위험도는 문법 자리가 정한다** — 같은 `field`가 수식어 자리에선 안전하고
    맨명사 자리에선 '분야'가 된다. §7의 `the floor`(주어 자리)와 같은 구조다.

### 8.6 B군(문법) 반영 — 24건 + note 7곳

관사·단복수·전치사 교정은 **역번역에 차이가 안 난다**. 그래도 영어에서는 오류이므로
역번역 결과만 보고 되돌리지 않는다. 역번역에서 드러나야 정상인 것만 따로 확인했다
(`답을 찾습니다` · `부하가 아니라 시스템이 제어` · `센서 → 센서 데이터` · `it → 결함` · `them → 개발자`).

되풀이된 것
- **제품명을 주어로 쓰면 '정유소'가 된다** — 현수분사를 고치며 `Refinery works with…`로
  바꿨더니 역번역이 '정유소는'이 됐다. `approach`·`note` 필드는 템플릿이 평문으로 출력해
  (`{u.approach}`) `<span translate="no">`를 넣을 수 없다. **주어 자리를 피하는 것이 유일한 방법**이다.
  `set:html`로 렌더하는 곳은 `useCases.subtitle` 하나뿐이다.
- **원문에 없는 제품명이 들어가 있었다** — 산업 7페이지 `note`의 `Refinery sits on top of it`은
  국문 원문("걷어내지 않고 그 위에 얹어")에 주어가 없다. 영어가 넣은 것이라 `a layer`로 바꿨다.
- **대시를 빼면 원문 구조로 돌아간다** — 영어가 대시로 합친 자리는 원문에서 마침표나 쉼표였다.
  블로그 4곳은 대시를 빼자 원문과 같은 두 문장이 됐다.
- **부사 하나가 역번역을 바꾼다** — `mostly fails`(비문) → `usually fails`는 '대개',
  `fails in most cases`도 '대개'였다. `most attempts … fail`로 주어에 올려야 '대부분'이 나왔다.

남은 em dash는 영어 페이지 본문 기준 **36곳 / 14페이지**다.

### 8.7 em dash 정리 — 22곳 제거, 4곳 유지

영어 페이지 본문의 em dash 26곳을 원문과 대조했다. **대시를 뺄 수 있는지는
영어 취향이 아니라 원문 구두점이 정한다** — 영어가 대시로 합친 자리는
원문에서 마침표나 쉼표였고, 빼면 원문 구조로 돌아간다.

| 유형 | 곳 | 되돌린 방식 |
|---|---|---|
| 원문이 쉼표였다 | 8 | 쉼표 |
| 원문이 두 문장·한 문장이었다 | 7 | 마침표 또는 콜론 |
| 어순을 원문대로 되돌리니 사라졌다 | 7 | `such as` · `whether` · 인용 앞당기기 |

**대시가 가리고 있던 누락 하나** — 유즈케이스 전력관리 이상 소비 카드는
원문이 두 문장("…잡습니다. …드러납니다.")인데 영어가 대시로 합치면서
두 번째 문장의 서술어 '드러납니다'가 통째로 빠져 있었다. 대시를 빼자 드러났다.

**유지한 4곳**
1. 랜딩 통합 섹션 **제목** — 두 선택지와 결론을 가르는 자리. 쉼표 셋이 이어지면 읽기 어렵다
2. 문서 프로토콜 목록 — 원문이 `안쪽 가운뎃점 + 무리 사이 쉼표`인데 영어는 가운뎃점을
   쉼표로 풀어서, 여기서 또 쉼표를 쓰면 두 무리의 경계가 사라진다(F군 가운뎃점 결정과 얽힘)
3. 유즈케이스 센서모니터링 — **문장 한가운데 삽입구**. 쉼표로 바꾸면 한 문장에 쉼표가 넷
4. 블로그 예지보전 — 원문이 `맥락, 곧 온톨로지`. 영어 `that is`를 넣으면 쉼표가 넷

### 8.8 이 라운드에서 배운 것

11. **번역기가 수식어를 옮겨 붙인다** — `with no manual steps`를 문장 끝에 두면
    '보고서로 자동 생성되며'처럼 **마지막 항목에만** 붙는다. 쉼표를 넣어야 전체를 수식한다.
12. **필요조건이 파악 대상으로 뒤집힌다** — `all connected together`가
    "어떻게 연결되어 있는지 파악해야"로 돌아왔다. 원문은 "연결돼 있어야"라는 조건이다.
13. **삽입구를 없애면 어순이 지켜진다** — 대시 삽입구를 문장 끝으로 빼면
...
### 8.11 D군(직역투) — 81곳, 4묶음으로 나눠 진행

D군은 다른 묶음과 성격이 다르다. A는 내용 누락, B는 문법, C는 업계 표기, E는 용어집
규칙 — 모두 **객관적으로 확인된다.** D의 "직역투"만 **한 사람의 인상**에 기댄다.

| 묶음 | 대상 | 제안 | 반영 |
|---|---|---|---|
| 1 | 랜딩·회사소개·에너지관리·블로그목록 | 8 | 6 |
| 2-A | 산업 7페이지가 공유하는 문장 | 5 | 5 |
| 2-B | 산업 페이지별 | 27 | 27 |
| 3 | 유즈케이스 | 7 | **2** |
| 4 | 블로그 | 7 | 3 (3건은 제안 전에 걸러냄) |

**유즈케이스만 적중률이 낮았다(2/7).** 그 묶음 항목은 대부분 CTA·태그라인처럼 6장
번역기 대조에서 이미 다듬은 문장이라, 원어민 검토자가 그것을 다시 짚은 셈이다.
반면 산업 7페이지 본문은 그때 덜 훑였고 실제 결함이 많이 나왔다
(`efficiency bends` · `the piece is out` · `held together` · `consumption lands`).

**전수 훑기로 목록 밖에서 더 나온 것들**
- `stacked`(식품 → 철강), `구간별로`(전자 2곳), `냉열`(기계 3곳)
- 카드 제목의 '설비' 누락(철강), 메타의 '배전' 누락(에너지), `each is tallied`(화학)

### 8.12 같은 낱말이라도 붙어 있는 유즈케이스를 먼저 본다

`냉열` 6곳을 `heating and cooling`으로 통일하려다 틀릴 뻔했다. 유즈케이스를 열어
보니 식품 3곳은 '냉동기 COP 계산'·'냉동기·압축기 운전 최적화'에 붙어 있어
`refrigeration`이 사실에 맞았고, 기계 3곳만 공조(HVAC) 유즈케이스였다.

→ **용어를 페이지 넘어 통일하기 전에 그 항목이 붙은 유즈케이스를 확인한다.**

### 8.13 D군 재점검 — 107개 구절 중 1곳 되돌림

요청 4항('말투나 문장은 원문과 동일한지')을 검증 기준에서 빠뜨린 것을 알고, 반영한
81곳을 107개 구절로 나눠 다시 훑었다.

| | 개수 |
|---|---|
| 누락 복원 · 비문 교정 · 원문 낱말 복원 · 표기 | 97 |
| 원문에서 멀어졌지만 되돌리면 오독·비문이 돌아오는 것 | 8 |
| 되돌림 | **1** |

되돌린 것은 식품의 `pinpoint` → `locate` 하나다. 원문 '짚어냅니다'에 강조가 없는데
`pinpoint`의 역번역이 '정확히 파악합니다'로 나왔고, `stacked`를 고치면서 같은 문장의
`locate`까지 덤으로 바꾼 것이라 근거가 없었다.

**되돌리려다 오히려 나빠진 것도 있다.** 에너지 `means checking`을 `requires`로
돌리고 `several`을 하나 빼 보니 `screens`가 화면이 아니라 '검토(screening)'로 읽혀
원문의 '화면'이 사라졌다. `checking`이 그 모호함을 막고 있었다.

### 8.14 말투 — 요청 4항을 뒤늦게 처리

요청에는 기준이 넷이었다. ①내용 누락·추가 ②문법·가독성 ③고유명사·용어·표현
④**말투나 문장이 원문과 동일한가**. ①④는 충실성, ②③은 품질 기준이고 서로 당긴다.

원어민 검토자에게 **원문 접근을 막았으니 ②③만 나온다.** 그 143건을 거를 때 내가
쓴 질문은 "제시안이 원문의 뜻을 지키는가"(①) 하나였고, **"지금 영어가 이미 원문의
낱말과 말투를 담고 있는가"(④)를 묻지 않았다.**

세션 첫 기계 검사에서 **원문이 서술인데 영어만 명령형인 곳 67곳**을 찾아 보고까지
해 놓고 A~E 묶음에 넣지 않았다. 뒤늦게 다섯 단계로 처리했다.
...
| ③ | 메타·태그라인 3 | 3 |
| ④ | 유즈케이스 how 카드 13개 | 13 |
| ⑤ | 유즈케이스 seo.description 7개 | 7 |

**수동태를 고른 이유** — 원문에 주어가 없다. 영어는 주어를 생략할 수 없으므로 수동이
무주어에 가장 가까운 대응이고, 산업 7페이지가 이미 그 방식이다. 사물 주어 능동으로
가면 `Refinery`가 주어 자리에 와 42곳에서 '정유소'로 읽힌다.

**오검출 두 가지**
- `Put X and Y`는 명령이 아니라 **조건문**(= '걸면 ~한다'). 원문 '걸면'과 대응한다
- 문단 끝 어미만 보면 원문이 '~하세요'인 것도 걸린다(블로그 품질예측의 `Pick`)

**판정 동사 목록을 좁게 잡아 3곳을 놓쳤다** — `Measure`·`Standardize`·`Monitor`가
빠져 유즈케이스 메타 7개 중 3개를 '이미 서술형'으로 잘못 분류했다.

### 8.15 F군 — 3건 반영, 4건 유지

**반영**
- **가운뎃점** 26단위 중 **산문 6곳만** 풀었다. 라벨·스펙 19단위는 표처럼 읽혀 덜
  걸리고, 건드리면 산업 7페이지 제목까지 번진다. `focuses on monitoring · quality`는
  영어로 `monitoring quality`(품질을 감시)로도 읽혀 뜻이 뒤집힐 수 있었다
- **`OT/IT` → `IT/OT`** 2곳, 영어만. 영어권 관용 어순이고 뜻은 같다. `OPC UA`처럼
  표준 기구가 정한 표기와 달리 언어별 관용이므로 국문은 두었다
- **`FEMS` → `Plant EMS`** 6곳, 영어만. 한·일권 약어라 미국 독자에게 통하지 않는다.
  사이트가 풀지 않고 쓰는 약어(SCADA·MES·ERP·PLC)는 모두 영어권 표준이다

**유지**
- `thermal fluid boiler` — 국문 표준어가 '열매체 보일러'다
- `read anomalies` — '읽다'는 사이트 전반의 브랜드 목소리다
- `the field` — 이미 유지로 결정했고 맨명사 자리 2곳만 `site`로 바꿨다
- 유즈케이스 카드 제목 7개 — 원문이 전부 명사구이고 명사구 정책도 직접 고른 것이다

**약어를 푸는 기준을 바로 알아냈다.** 처음에 "현장 장비는 그냥 쓰고 제품 개념은
푼다"고 정리했는데 `COP`가 반례였다. 괄호로 푼 10곳을 전부 열어 보니 **예외 없이 그
용어를 정의·설명하는 문장**이었다(`What is an energy management system (EMS)?`).
나열·지칭하는 자리는 약어만 쓴다. 장비냐 개념이냐는 상관이 없었다.

### 8.16 제품명 래핑 — 자리마다 가능 여부가 다르다

`Refinery`가 문장 첫머리에 오면 번역기가 일반명사로 읽어 '정유소'가 된다. 사이트
전체 래핑은 **20곳 16페이지**가 됐다(랜딩 본문 2 · 산업 subtitle 7 · 유즈 CTA 7 ·
FAQ 3 · 회사소개 lede 1).

**넣었다가 사고가 나서 되돌린 곳**
- 회사소개 `description` — 이 필드는 **메타 설명**이라 span이 속성값 안으로 들어가
  `content="&lt;span translate=..."`로 새어 나왔다
- 에너지관리·산업AI FAQ 3곳 — `PillarPage`가 FAQ 답변을 **평문으로 출력**하고
  (`{f.a.split('\n').map(...)}`) 같은 필드를 **FAQPage JSON-LD에도 그대로** 넣는다.
  화면에 `&lt;span ...&gt;`이 글자로 보이고 구조화 데이터까지 오염된다

→ **래핑 전에 그 필드가 `set:html`인지, 메타·JSON-LD로도 쓰이는지 확인한다.**
같은 이름의 `description`이라도 자리에 따라 다르다.

**감쌀 수 없어 받아들인 자리**
| 자리 | 곳 | 이유 |
|---|---|---|
| 메타 `pageTitle`·`description` | 15 | 속성값이라 HTML 불가 |
| `approach`·`note` 필드 | — | 평문 출력. 주어 자리를 피해 해결했다 |
| PillarPage FAQ 답변 | 4 | 평문 출력 + JSON-LD. 원문이 'Refinery는'을 주어로 쓰므로 유지 |

### 8.17 이 라운드에서 배운 것

15. **역번역은 말투를 못 잡는다** — 한국어 경어 정보가 영어에 없어 번역기가 장르를
    보고 지어낸다. 짧으면 합쇼체, 길면 해라체로 갈리고, 명령형도 문장이 길면 서술로
    의역한다. 말투는 **영어가 주어 없이 동사로 시작하는지**를 직접 보는 편이 확실하다.
16. **역번역이 원문과 다르면 넘기지 않는다** — 2-A에서 `tallied`를 고치며 '집계'를
    '계량'으로 바꿨다. 그때 역번역이 '계측되는'으로 나왔는데 넘겼고, 화학 본문을
    보다 뒤늦게 잡아 2곳을 되돌렸다.
17. **한 낱말을 지키려면 한국어 쪽 연어를 본다** — `peaks`가 '급증'으로 풀리는 걸
    막으려 관사를 되살리니 '업무량 급증'으로 더 나빠졌다. `demand peaks`도 '수요
    급증'이었고, `power peaks`만 '전력 피크'로 살아남았다. 한국어에서 굳어진 짝이라야
    번역기가 낱말을 그대로 둔다.
18. **고치려던 것만 고친다** — 말투를 바꾸며 관사가 딸려 나가고, `stacked`를 고치며
    `locate`가 딸려 나갔다. 수정 단위를 좁게 잡아야 재점검 부담이 줄어든다.
19. **필터는 오검출을 낸다** — 명령형 판정에서 조건 구문(`Put X and Y`)과 원문이
    이미 명령인 것이 걸렸고, 동사 목록이 좁아 3곳을 놓쳤다. 목록으로 좁힌 뒤에는
    **각 항목을 열어 봐야** 한다.
